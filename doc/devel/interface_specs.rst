...
NiftyPipe is a young project maintained by an enthusiastic group of developers.
Even though the documentation might be sparse or cryptic at times we strongly
encourage you to contact us on the official niftypipe developers mailing list in
case of any troubles: nipy-devel@neuroimaging.scipy.org (we are sharing a
mailing list with the nipy community therefore please add ``[niftypipe]`` to the
messsage title).
...
advantage of.  But don't be intimidated!  To write a NiftyPipe Trait
...
* If your interested in more of a *big picture* overview, `Gael wrote
  ...
  the benefit of the generated UI components.  (For now, NiftyPipe is not
...
NiftyPipe Interface Specifications
-------------------------------
...
to Bet.  If you are familiar with the NiftyPipe 0.2 code-base, these
...
``inputs`` appear to a user for Bet::
  ...
  >>> from niftypipe.interfaces import fsl
  >>> bet = fsl.BET()
  >>> type(bet.inputs)
  <class 'niftypipe.interfaces.fsl.preprocess.BETInputSpec'>
  >>> bet.inputs.<TAB>
...
base class down to subclasses).:
  ...
  ``TraitedSpec``: NiftyPipe's primary base class for all Specs.
  Provides initialization, some niftypipe-specific methods and any trait
...
(defined in ``niftypipe.interfaces.base``) check for the existence/or
...
``exists``
	For files, use ``niftypipe.interfaces.base.File`` as the trait type.  If
	...
	be presented with an error message::
	    ...
	    Traceback (most recent call last):
	      ...
	      File "/Users/cburns/local/lib/python2.5/site-packages/niftypipe/interfaces/base.py", line 76, in validate
...
``hash_files``
	...
	is set to false any NiftyPipe will not try to hash any files described by this
...
``desc``
	...
	**Required:** This metadata is required by all niftypipe interface
	  classes.
...
``usedefault``
	...
	NiftyPipe only passes ``inputs`` on to the underlying package if they
...
``argstr``
	...
	dictionaries of NiftyPipe 0.2 code.  If we look at the
...
``genfile``
	...
	one.  The niftypipe convention is to automatically generate output
...
* ``_gen_filename(name)``:  Generate filename, used for filenames that niftypipe generates as a convenience for users.  This is for parameters that are required by the wrapped package, but we're generating from some other parameter. For example, ``BET.inputs.outfile`` is required by BET but we can generate the name from ``BET.inputs.infile``.  Override this method in subclass to handle.

And optionally:

* ``_redirect_x``: If set to True it will make NiftyPipe start Xvfb before running the interface and redirect X output to it. This is useful for
...
check our inputs to see which are defined and which are not::
    ...
    >>> bet.cmdline
    'bet f3.nii /Users/cburns/data/niftypipe/s1/f3_brain.nii.gz'
