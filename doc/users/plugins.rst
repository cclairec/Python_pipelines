...
In order to use niftypipe with SGE_ or PBS_ you simply need to call::
...
submitting entire graph of dependent jobs at once. This way NiftyPipe does not
...
With the ``CondorDAGMan`` plug-in NiftyPipe can utilize this functionality to
...
instantaneously -- NiftyPipe is only used to generate the workflow graph,
...
NiftyPipe, only HTCondor pools with a shared file system can be used to process
NiftyPipe workflows.
...
Using the ``wrapper_cmd`` argument it is possible to combine NiftyPipe workflow
...
In order to use niftypipe with OAR_ you simply need to call::
