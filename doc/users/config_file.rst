...
Some of the system wide options of NiftyPipe can be configured using a
configuration file. NiftyPipe looks for the file in the local folder under the name
``niftypipe.cfg`` and in ``~/.niftypipe/niftypipe.cfg`` (in this order). If an option
...
*try_hard_link_datasink*
	...
	of niftypipes internal cache structure, a file system hard link will be
...
*job_finished_timeout*
    ...
    externally. NiftyPipe checks to see if a results file exists to determine if
...
::
    ...
    import os
    from niftypipe import config, logging
