...
def grab_coverage(output):
    ...
    tcount = None
    for line in output:
        if line.startswith('niftypipe.interfaces.') or \
                line.startswith('niftypipe.pipeline.') or \
                line.startswith('niftypipe.utils.'):
...
def main():
    cmd = 'nosetests --with-coverage --cover-package=niftypipe'
