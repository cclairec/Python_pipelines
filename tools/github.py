...
def is_git_repo():
    """Does the current niftypipe module have a git folder
    """
    sourcepath = os.path.realpath(os.path.join(os.path.dirname(niftypipe.__file__),
                                               os.path.pardir))
...
def get_local_branch():
    ...
    if is_git_repo():
        o, _ = Popen('git branch | grep "\* "', shell=True, stdout=PIPE,
                     cwd=os.path.dirname(niftypipe.__file__)).communicate()
        return o.strip()[2:]
    else:
        return None
...
def create_hash_map():
    ...
    conn = http.client.HTTPSConnection("cmiclab.cs.ucl.ac.uk")
    conn.request("GET", "/CMIC/NiftyPipe",
                 headers={'Authorization': 'Basic %s' % base64(login_name)})
    try:
        conn.request("GET", "/CMIC/NiftyPipe/tree/master?recursive=1")
    except:
        pass
    else:
        r1 = conn.getresponse()
...
def get_repo_url(force_github=False):
    ...
    if not os.path.exists(gitpathgit) and not force_github:
        ...
        uri = 'https://cmiclab.cs.ucl.ac.uk/CMIC/NiftyPipe/blob/master'
    return uri
...
def get_file_url(object):
    ...
    if is_git_repo():
        info = niftypipe.get_info()
        shortfile = os.path.join('niftypipe', filename.split('niftypipe/')[-1])
        uri = 'https://cmiclab.cs.ucl.ac.uk/CMIC/NiftyPipe/tree/%s/%s#L%d' % \
            (info['commit_hash'],
             shortfile, lines[1])
    return uri
