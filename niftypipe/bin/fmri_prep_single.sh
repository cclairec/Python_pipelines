...
for run in $runs; do
    ...
    3dcopy pb03.r$run.volreg+orig fmri_motion_corrected.nii.gz
...
reg_aladin -ref min_outlier_volume.nii.gz -flo ${anat} -rmask full_mask.nii.gz -fmask T1_brain_mask.nii.gz -aff anat_2_fmri_aff.txt -res anat_in_fmri_space.nii.gz
...
if [ ! -z "${seg}" -a "${seg}" != " " ] && [ ! -z "${atlas}" -a "${atlas}" != " " ]; then
  ...
  reg_resample -ref min_outlier_volume.nii.gz -flo $atlas -trans anat_2_fmri_aff.txt -res atlas_fmri.nii.gz -inter 0
...
cp -f seg_wm_fmri.nii.gz ../seg_wm_fmri.nii.gz
cp -f seg_gm_fmri.nii.gz ../seg_gm_fmri.nii.gz
cp -f seg_csf_fmri.nii.gz ../seg_csf_fmri.nii.gz
cp -f dfile_rall.1D ../dfile_rall.1D
cp -f outcount_rall.1D ../outcount_rall.1D
cp -f motion_enorm.1D ../enorm.1D
cp -f atlas_fmri.nii.gz ../atlas_fmri.nii.gz
cp -f fmri_motion_corrected.nii.gz ../fmri_motion_corrected.nii.gz
